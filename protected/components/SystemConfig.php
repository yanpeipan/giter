...
class SystemConfig 
{
	...
	//返回id
	static public function GetCateId($value){
		$sql = "SELECT id FROM {{".self::$_tableName."}} WHERE cfg_type='USER' AND cfg_name='VIDEO_CATEGORY' AND cfg_value=:cfg_value";
		$cmd = Yii::app()->db->createCommand($sql);
		$cmd->bindvalues(array(':cfg_value'=>$value));
		$row = $cmd->queryRow();
		return $row['id'];
	}
