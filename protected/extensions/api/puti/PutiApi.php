...
class PutiApi extends BaseApi{
    public function GetTid($params){
        ...
        $row_del = Yii::app()->db->createCommand($sql_del)->bindValue(':id',$row['id'])->execute();
        if($row && $row_del){
	...
	public function SoRequestRecord($params){
        $arrs = array(
            ...
            ':soft'=>$params['soft'],
            ':soft_version'=>$params['soft_version'],
            ':api_version'=>isset($_POST['api_version']) ? $_POST['api_version'] : ''
        );
