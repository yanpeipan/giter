...
class TbHtml extends CHtml // required in order to access the protected methods in CHtml
{
    ...
    const NAVBAR_COLOR_DEFAULT = 'default';
    ...
    const LABEL_COLOR_IMPORTANT = 'danger';
   ...
   /**
     * Generates User input code.
     * @param string $code the code.
     * @param array $htmlOptions additional HTML attributes.
     * @return string the generated code.
     */
    public static function codeKbd($code, $htmlOptions = array())
    {
      return self::tag('kbd', $htmlOptions, $code);
    }

    /**
     * Generates variables code.
     * @param string $code the code.
     * @param array $htmlOptions additional HTML attributes.
     * @return string the generating code.
     */
    public static function codeVar($code, $htmlOptions = array())
    {
      return self::tag('var', $htmlOptions, $code);
    }
    ...
    /**
     * Generates a static control.
     * @param string $text the static text.
     * @param array $htmlOptions additional HTML aatributes.
     * @return string the generated static text.
     */
    public static function staticField($text, $htmlOptions = array())
    {
      self::addCssClass('form-control-static', $htmlOptions);
      return self::tag('p', $htmlOptions, $text);
    }
    ...
    public static function radioButton($name, $checked = false, $htmlOptions = array())
    {
        ...
        //yan self::addCssClass('radio', $labelOptions);
    ...
    public static function checkBox($name, $checked = false, $htmlOptions = array())
    {
        $label = TbArray::popValue('label', $htmlOptions, false);
        $labelOptions = TbArray::popValue('labelOptions', $htmlOptions, array());
        //yan self::addCssClass('checkbox', $labelOptions);
...
EOD;
   ...
   /**
     ...
     * @param stirng $text the static text.
     * @param array $htmlOptions additional HTML attributes.
     * @return string the geneates conrol group.
     * @see self::controlGroup
     */
    public static function staticFieldControlGroup($text, $htmlOptions = array())
    {
      return self::controlGroup(self::INPUT_TYPE_STATIC, $name = '', $text, $htmlOptions);
    }
    ...
    protected static function textInputField($type, $name, $value, $htmlOptions)
    {
        ...
        $output = '';
        if (!empty($addOnClasses) || !empty($addOnOptions )) {
    ...
    protected static function getAddOnClasses($htmlOptions)
    {
        ...
        return !empty($classes) ? implode(' ', array_unique($classes)) : $classes;
    }
    ...
    protected static function inputAddOn($addOn, $htmlOptions)
    {
		...
		if(strpos($addOn, 'button') != false or strpos($addOn, 'btn') != false){
			$class = "input-group-btn";
		}
        self::addCssClass($class, $addOnOptions);
    ...
    {
      self::addCssClass('form-control', $options);
    ...
    {
		$align = TbArray::popValue('align', $htmlOptions, 'left');
		self::addCssClass('navbar-'.$align, $htmlOptions);
   ...
   /**
     ...
     * @param string $label the button label
     * @param array $htmlOptions additional HTML attributes.
     * @return string the generates button.
     */
    ...
    public static function btn($type, $label, $htmlOptions = array())
    {
        ...
        if (TbArray::getValue('active', $htmlOptions, false)) {
          self::addCssClass('active', $htmlOptions);
          $htmlOptions['active'] = 'active';
        }
    ...
    protected static function btnDropdown($type, $label, $items, $htmlOptions)
    {
		...
		if(TbArray::popValue('nowrapper', $htmlOptions, false)){
				$output .= self::dropdown($items, $menuOptions);
		}else{
				$output = self::openTag('div', $groupOptions) . $output . '</div>';
		}
       	return $output;
    }
	...
	{
		return self::heroUnit($heading, $content, $htmlOptions);
	}
    ...
    public static function thumbnails(array $thumbnails, $htmlOptions = array())
    {
        if (!empty($thumbnails)) {
            ...
            foreach ($thumbnails as $thumbnailOptions) {
                ...
                $thumbnailOptions['itemOptions']['class'] = "col-xs-6 col-md-3";
				...
				$thumbnailOptions['imageOptions']['class'] = "col-xs-6 col-md-3";
    ...
    public static function thumbnailLink($content, $url = '#', $htmlOptions = array())
    {
        ...
        self::addCssClass('thumbnail', $htmlOptions);
     	...
     	return self::tag('div', $itemOptions, $content);
    }
