...
class PluginManager 
{
	...
	static public function GetPluginConfig($pluginid,$cache=true,$dir=null)
	{
		...
		$pluginconfigfile = $dir ."/".$pluginid.".php";
	...
	static public function GetPlugins($type="all",$page=1,$pageSize=20)
	{
		...
		if("setuped"==$type){
			...
			$pluginDir = Yii::app()->modulePath."/plugin/controllers/";
			$fileArray = array_slice(scandir($pluginDir,0),2);//过滤掉.|..目录
			$fileArray = array_map(function($pluginController){return strtolower(substr($pluginController,0,strpos($pluginController, "Controller")));},$fileArray);
			//改写fileArray
			if("new" == $type){
		...
		if(!empty($fileArraySlice)){
			foreach($fileArraySlice as $pluginid){
				...
				$pluginconfigfile = self::GetPluginPath($pluginid)."/".$pluginid.".php";
				if(is_file($pluginconfigfile)){
	...
	{
		return Yii::app()->modulePath."/plugin/src/".strtolower($pluginid);
	}
	...
	static public function ParsePluginConfig($pluginid,$conf=null)
	{
		if(is_array($conf)){
			...
			$configfile = self::GetPluginPath($pluginid)."/".$pluginid.".php";
	...
	{
		$data = array('status'=>self::STATUS_SUCCESS,'msg'=>'删除成功');
		//delete {$pluginid}Controller.php
		$pluginController = Yii::app()->modulePath."/plugin/controllers/".ucfirst($pluginid)."Controller.php";
		$fileHanlder = Yii::app()->file->set($pluginController,true);
		$f = $fileHanlder->delete();
		//delete /plugin/src/{$pluginid}
		$pluginDir = self::GetPluginPath($pluginid);
		$dirHanlder = Yii::app()->file->set($pluginDir,true);
		$d = $dirHanlder->delete(true);
		if(!($f && $d)){
			$data['status'] = self::STATUS_ERROR;
			$data['msg']    = "删除失败(没有权限)，请手动删除插件相关文件和目录！";
		}
		return $data;
	}
	...
	static public function update($pluginid)
	{
		...
		if($config){
			...
			if($newVersion > $oldVersion){
				//更新配置文件
				$conffile = $updateDir.strtolower($pluginid).".php";
				$destconffile = self::GetPluginPath($pluginid)."/".strtolower($pluginid).".php";
				if(is_file($conffile)){
					$confHandler = Yii::app()->file->set($conffile);
					$s = $confHandler->copy($destconffile);
					if(!$s){
						$data['status'] = self::STATUS_ERROR;
						$data['msg']    = "更新失败，没有权限移动升级文件，请修正权限";
						return $data;
					}
				}
				//更新controller
				...
				if(is_file($controller)){
					$controllerHandler = Yii::app()->file->set($controller);
					$s = $controllerHandler->copy($destController);
					if(!$s){
						$data['status'] = self::STATUS_ERROR;
						$data['msg']    = "更新失败，没有权限移动升级文件，请修正权限";
						return $data;
				...
				if(is_dir($viewsDir)){
					$DirHandler = Yii::app()->file->set($viewsDir);
					$s = $DirHandler->copy(self::GetPluginPath($pluginid)."/views");
					if(!$s){
						$data['status'] = self::STATUS_ERROR;
						$data['msg']    = "更新失败，没有权限移动升级文件，请修正权限";
						return $data;
				...
				if(is_dir($libDir)){
					$DirHandler = Yii::app()->file->set($libDir);
					$s = $DirHandler->copy(self::GetPluginPath($pluginid)."/lib");
					if(!$s){
						$data['status'] = self::STATUS_ERROR;
						$data['msg']    = "更新失败，没有权限移动升级文件，请修正权限";
						return $data;
				...
				//删除升级目录
				Yii::app()->file->set($updateDir)->delete(true);
			}else{
